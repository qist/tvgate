...
每次通过 Web 后台**保存任意配置**（server / http / web / monitor / php / sync / github / global-auth / log / proxygroups / ts/multicast 等）时，都会先把当前旧配置完整写到同目录的 **`config.yaml.backup.<时间戳>`**（时间戳格式 `20060102150405`，如 `config.yaml.backup.20260827150405`），再覆盖写入新配置。

- 备份文件与 `config.yaml` 同目录，可用于手工回滚。
- Web 后台提供**配置备份中心**（`/web/config/backup`）：`列表 / 删除 / 批量删除 / 还原 / 下载`，按时间从新到旧排序；无自动清理，需要手动删或按需定期清理。

### 2. 代码中心（代码文件管理）备份

在**代码文件管理器**里每次**保存/批量替换**文件时，先把旧内容复制为 **`原文件.bak.<时间戳>`**（时间戳格式 `YYYY-MM-DD_HH-MM-SS`，如 `test.php.bak.2026-08-27_22-28-00`，与原文件同目录），再写新内容。

- Docroot 下所有 `.bak.<时间戳>` 文件由**备份文件管理中心**（入口在代码编辑页）统一管理，支持：`列表`（递归扫描 docroot 下 `.bak.`）、`回滚`（恢复时也会先把当前文件再备份一份）、`下载`、`单个/批量删除`、`自动清理`（按原文件分组保留最新 N 个，`keep` 可设 0=全删）。
- 回滚操作本身也会生成一条新备份，避免覆盖不可逆。
