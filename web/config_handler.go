...
func (h *ConfigHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc(webPath+"api/publisher/ffmpeg", h.cookieAuth(h.handlePublisherFFmpegStatus))
	...
	mux.HandleFunc(webPath+"config/save-multicast", h.cookieAuth(h.handleMulticastConfigSave))
	mux.HandleFunc(webPath+"config/ts", h.cookieAuth(h.handleTSConfig))
	mux.HandleFunc(webPath+"config/save-ts", h.cookieAuth(h.handleTSConfigSave))
