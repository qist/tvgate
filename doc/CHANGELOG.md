...
```
1、修复 AC-3/E-AC-3 软解长播"声音越播越超前" — 根因：不同源会对音频 PES PTS
   做周期性再定时（实测 256ms/1024ms/5120ms，均为 AC-3 帧长 32ms 整数倍），旧
   阈值（100ms）把这些伪跳变误判为"不连续"→ 重锚 + bridging 压缩音频时间轴 →
   每窗口声音超前 K ms、播放越久越明显。现把音频重锚阈值提到 10s：正常播放恒定
   在连续样本时钟上，仅对 ≥10s 的切台/节目级真不连续才重锚，速率差交由主线程
   漂移控制器平滑吸收
2、漂移控制改"比例+抗饱和积分"并收敛增益/幅度 — 原比例控制对大速率差留下稳态
   残差，先前引入的大增益积分又引发限幅振荡（±100ms 摆动）。现积分降为慢速微调，
   软同步幅度收敛，闭环稳定（长播验证 drift 收敛到几 ms、稳定走 WSOLA 直通）
3、A/V 显示延迟自适应补偿（rVFC，无写死值）— video.currentTime 是解码时钟，
   画面真正上屏滞后若干帧且逐设备/频道不同（4K HEVC 管线深浅、硬解能力差异，
   静态全局值无法兼容）。现用 requestVideoFrameCallback 逐帧实测
   currentTime-mediaTime 得到本频道/设备的真实显示滞后（滑窗 32 帧取最小值 +
   合成→出光修正），音频动态贴齐画面；seek/暂停不采样、异常样本钳制、不支持
   rVFC 的浏览器自动保持 0（行为同旧版）。漂移目标/硬重对齐/恢复重对齐/链重启
   对齐全部接入实测值
```
