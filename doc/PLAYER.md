...
- 台标是外部资源：服务端只做地址补齐（源 `tvg-logo` → 本地 `logo_dir` → `logo` 模板），不保证图床真有对应图片。订阅重载时会并发探测全部外部台标地址（限并发、单轮 10s 总预算），确认 404/410 的死链直接置空——前端立即回落首字徽章，不再逐个空等图片加载失败；403/5xx/网络错误视为不确定（可能防盗链），保留原地址。探测结论按地址缓存（命中 24h / 缺失 2h），稳态刷新零探测。
