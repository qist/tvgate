...
func defaultPHPConsts() map[string]Value {
	...
	// PCRE 常量（preg_match_all/preg_split/preg_grep 的 flags 与错误码，PHP 语义）
	c["PREG_PATTERN_ORDER"] = NewInt(1)
	c["PREG_SET_ORDER"] = NewInt(2)
	c["PREG_OFFSET_CAPTURE"] = NewInt(256)
	c["PREG_UNMATCHED_AS_NULL"] = NewInt(512)
	c["PREG_SPLIT_NO_EMPTY"] = NewInt(1)
	c["PREG_SPLIT_DELIM_CAPTURE"] = NewInt(2)
	c["PREG_SPLIT_OFFSET_CAPTURE"] = NewInt(4)
	c["PREG_GREP_INVERT"] = NewInt(1)
	c["PREG_NO_ERROR"] = NewInt(0)
	c["PREG_INTERNAL_ERROR"] = NewInt(1)
	c["PREG_BACKTRACK_LIMIT_ERROR"] = NewInt(2)
	c["PREG_RECURSION_LIMIT_ERROR"] = NewInt(3)
	c["PREG_BAD_UTF8_ERROR"] = NewInt(4)
	c["PREG_BAD_UTF8_OFFSET_ERROR"] = NewInt(5)
	c["PREG_JIT_STACKLIMIT_ERROR"] = NewInt(6)
	// PHP 常量
	c["PHP_EOL"] = NewString("\n")
