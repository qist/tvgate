...
import { MP3Data, type MP3Frame, MP3Parser } from "./mp3";
...
class TSDemuxer {
  ...
  /** 上一帧的时长（ms）：PES 头部被半帧占用时，用它接续 PTS 外推基准。 */
  private audio_last_frame_duration_ms_: number | undefined = undefined;
  private aac_last_incomplete_data_: Uint8Array | null = null;
  private ac3_last_incomplete_data_: Uint8Array | null = null;
  private mp2_last_incomplete_data_: Uint8Array | null = null;
  ...
  public destroy() {
    ...
    this.mp2_last_incomplete_data_ = null;
  ...
  private resetAudioParserState(): void {
    ...
    this.audio_last_frame_duration_ms_ = undefined;
    this.aac_last_incomplete_data_ = null;
    this.ac3_last_incomplete_data_ = null;
    this.eac3_last_incomplete_data_ = null;
    this.mp2_last_incomplete_data_ = null;
      ...
      if (stream_type === StreamType.kH264 && !already_has_video) {
        ...
        for (let offset = i + 5; offset < i + 5 + ES_info_length;) {
          ...
          if (tag === 0x05) {
            ...
            // ATSC AC-3(0x82) / ETSI AC-3 descriptor(0x6A, DVB)
  ...
  private parseMP3Payload(data: Uint8Array, pts: number | undefined) {
    ...
    // MP2 software decode: 与 AC-3/E-AC-3 同一模型 —— 在 demuxer 里按帧切分、逐帧外推
    // PTS，每帧带自己的 PTS 送 WASM；跨 PES 的半帧由 mp2_last_incomplete_data_ 兜底。
    ...
    if (this.audio_drop_until_sync_) {
      if (!sync_at_start) {
        // 不连续之前残留的半帧属于旧时间轴，不能拼到新 payload 前面
        this.mp2_last_incomplete_data_ = null;
        return;
      }
      this.audio_drop_until_sync_ = false;
    ...
    if ((object_type === 33 || soft_decode_active) && this.onRawAudioData) {
      ...
      // 逐帧送出（元数据 / audio init 段由首个完整帧派发）
      this.parseMP2Frames(data, pts);
  ...
  /**
   * MP2 软解主路径：**按帧切分 + 逐帧 PTS 外推**（与 AC-3/E-AC-3 同一模型）。
   *
   * - 帧长由帧头自身给出，跨 PES 的半帧由 `mp2_last_incomplete_data_` 兜底，保证进
   *   WASM 的始终是完整帧（WASM 侧 parser 的 one-frame 延迟由 pipeline 用
   *   `samplesBeforeInput` 回退到帧起点，标签仍逐帧正确）；
   * - PES PTS 只作基准；payload 头部被上一段半帧占用时按「上一帧 PTS + 上一帧时长」
   *   接续，避免把 PES PTS 误当成当前帧起点；
   * - 单帧异常只跳过该帧并照常推进 PTS，保持时间轴连续（与 AC-3 一致）。
   */
  private parseMP2Frames(data: Uint8Array, pts: number | undefined): void {
    const had_incomplete_data = this.mp2_last_incomplete_data_ !== null;
    if (this.mp2_last_incomplete_data_) {
      const buf = new Uint8Array(data.byteLength + this.mp2_last_incomplete_data_.byteLength);
      buf.set(this.mp2_last_incomplete_data_, 0);
      buf.set(data, this.mp2_last_incomplete_data_.byteLength);
      data = buf;
    }

    let base_pts_ms: number | undefined;
    if (pts !== undefined) {
      base_pts_ms = pts / this.timescale_;
    }
    if (had_incomplete_data && this.audio_last_sample_pts_ !== undefined) {
      const last_duration_ms = this.audio_last_frame_duration_ms_;
      if (last_duration_ms !== undefined) {
        const new_pts_ms = this.audio_last_sample_pts_ + last_duration_ms;
        if (base_pts_ms === undefined || Math.abs(new_pts_ms - base_pts_ms) > 1) {
          base_pts_ms = new_pts_ms;
        }
      }
    }
    if (base_pts_ms === undefined) {
      Log.w(this.TAG, `MP2: Unknown pts`);
      return;
    }

    const mp3_parser = new MP3Parser(data);
    let mp3_frame: MP3Frame | null = null;
    try {
      mp3_frame = mp3_parser.readNextMP3Frame();
    } catch (e) {
      Log.w(this.TAG, `MP2 parse aborted (readNextMP3Frame): ${(e as Error).message}`);
      mp3_frame = null;
    }
    if (mp3_frame != null) {
      this.audio_drop_until_sync_ = false;
    ...
    let sample_pts_ms = base_pts_ms;
    let last_sample_pts_ms: number | undefined;
    let last_frame_duration_ms: number | undefined;

    while (mp3_frame != null) {
      let frameOk = false;
      let ref_sample_duration = 24; // 兜底：48kHz / 1152 采样一帧
      try {
        ref_sample_duration = (mp3_frame.samples_per_frame / mp3_frame.sampling_frequency) * 1000;

        const mp3sample = new MP3Data();
        mp3sample.object_type = mp3_frame.object_type;
        mp3sample.sample_rate = mp3_frame.sampling_frequency;
        mp3sample.channel_count = mp3_frame.channel_count;
        mp3sample.data = mp3_frame.data;
        ...
        if (this.audio_init_segment_dispatched_ === false) {
          this.audio_metadata_ = {
            ...
            sample_rate: mp3_frame.sampling_frequency,
        ...
        last_sample_pts_ms = sample_pts_ms;
        last_frame_duration_ms = ref_sample_duration;
        this.onRawAudioData?.({ codec: "mp2", data: mp3_frame.data, pts: Math.floor(sample_pts_ms) });
        frameOk = true;
      } catch (e) {
        // 单帧（畸形帧 / 源流 PTS 重叠）解析异常不应打断整个分片：跳过坏帧并保持 PTS 连续
        Log.w(this.TAG, `MP2 frame dropped (parse error): ${(e as Error).message}`);
      }
    ...
    // getIncompleteData() 在无残帧时返回 null —— 必须每次都赋值，否则上一次的残帧
    // 会被重复拼到后续 payload 前面
    this.mp2_last_incomplete_data_ = mp3_parser.getIncompleteData();

    if (last_sample_pts_ms !== undefined) {
      this.audio_last_sample_pts_ = last_sample_pts_ms;
      this.audio_last_frame_duration_ms_ = last_frame_duration_ms;
    }
  }
